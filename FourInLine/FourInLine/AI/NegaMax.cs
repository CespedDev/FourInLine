    ...
    public class NegaMax : IAI
    {
        ...
        public int MakeDecision(Board board)
        {
            ...
            foreach(int col in board.PosiblesInsert())
            {
                ...
                //Recurse.
                int recursedScore = -NegaMaxAB(new Board(newBoard), depth, -beta, -alpha);
        ...
        private int NegaMaxAB(Board board, int maxDepht, int alpha, int beta, int currentDepth = 0)
        {
            ...
            foreach(int col in board.PosiblesInsert())
            {
                Board newBoard = new Board(board, col);
                //Recurse
                int recursedScore = -NegaMaxAB(new Board(newBoard), maxDepht, -adaptiveBeta, -Math.Max(alpha, bestScore), currentDepth + 1);
                int currentScore = -recursedScore;
                ...
                if(currentScore>bestScore)
                {
                    ...
                    {
                        int negativeBestScore = -NegaMaxAB(new Board(newBoard), maxDepht, -beta, -currentScore, currentDepth);
